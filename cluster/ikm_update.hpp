...
)
{
	...
	for(int j = 0, g = c * c; j < g; ++j)
	{
		...
		if(!q || k >= K || skip[k]) continue;   // skip unassigned or empty cells
		p[k] += P[j];                           // contribute to centroid population
		s[k] += q * Z[j];                       // contribute to centroid variance
		float *Wk = W + D * k;                  // offset of k-th centroid
		float const *Mj = M + D * Mi[j];        // offset of j-th cell mean
		for(int d = 0; d < D; ++d)              // contribute to centroid mean
			Wk[d] += q * Mj[d];
	}
